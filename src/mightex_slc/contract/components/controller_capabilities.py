...
class ControllerCapabilities(ContractModel):
    ...
    These fields are device truth sent server-to-client, so the bounds below
    are sanity guards on a trusted source, not request validation.
    """
    ...
    current_resolution_ma: float = Field(
        gt=0,
        description=(
            "Current step size in milliamps. The vendor documents state 1.0 "
            "for AA, AV, SA, SV, HA, HV, and MA modules, 5.0 for CA, and 0.1 "
            "for FA, FV, XA, and XV. They state no resolution for QA, so any "
            "QA value is a server-side library convention, not a device fact."
        ),
    )
    max_profile_steps: int = Field(
        ...
        description=(
            ...
            "pair. The vendor documents give 127 for full-profile modules and "
            "as few as 2 for modules they call limited."
        ),
    )
