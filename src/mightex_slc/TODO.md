...
Follow-up notes for reviewing and refining the TRIGGER-mode implementation.
...
- [ ] Following the related question in `profiles.py`, investigate whether the
...
- [ ] Revisit the trigger-profile encoding and parsing implementation; parts
  of the current approach are not fully satisfactory.
...
- [ ] Tighten the docstrings for `set_trigger_profile()` and
  `get_trigger_profile()`.
  - Reconsider the direct use of `exchange_multiline()` inside
    `get_trigger_profile()`; it feels more like a workaround than a clean
    design.
  - File: `src/mightex_slc/transport/rs232/rs232_transport.py`

## Serial link

- [ ] Revisit the multiline exchange mechanism; parts of it feel more like a
  workaround than a clean design.
  ...
  - A single, unified exchange function that supports both ordinary and
    multiline responses is strongly preferred.
