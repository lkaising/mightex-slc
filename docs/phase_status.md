...
| 1 | Contract foundation: port six operations, six components, two bases; write schema generator; generate schemas; staleness test | `import mightex_slc.contract` works; each operation's valid request round-trips, invalid rejected; schemas generated and staleness test green | complete | 2026-07-04. Complete under a revised Phase 1 / Phase 1.5 split: contract models, schema generator, and generated schemas implemented; smoke verification passed (`import mightex_slc.contract` works, all six valid requests round-trip via `model_dump(mode="json")`, a reply union validates both branches, and invalid requests reject: set > max, negative current, channel < 1, missing `device_id`). The generator emits each shared shape once in its component file and cross-references it, with a consistency check on every run. Formal contract tests and the schema staleness test — part of this gate as originally written — are deferred to Phase 1.5; the original gate did not pass unchanged. Open investigation unchanged: `NormalParameters` read-path-only vs. reuse in `configure_normal` (plan §6, Phase 1); `normal_parameters.py` stays a dormant stub. |
